...
The strategic goal is to evolve the `teddy` executor's plan format from YAML to Markdown. This change is intended to improve the readability and authoring experience of AI-generated plans, making the system more transparent and user-friendly.
...
-   [New Plan Format Specification](/docs/specs/new-plan-format.md)
...
-   **Backwards Compatibility:** The `YamlPlanParser` must be preserved to ensure full backwards compatibility for existing workflows and test cases. The system should be designed to handle both formats seamlessly until the new parser is fully validated.
-   **Testing Strategy:**
...
-   [ ] **Deprecate Legacy Logic:** Once all tests are passing and the new parser is confirmed to be working correctly, the old YAML parsing logic (e.g., handling colons) can be deprecated and removed.
