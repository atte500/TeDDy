...
└── sessions/
    └── 20260124-add-user-auth/      # Session Directory
        ...
        └── 01/                      # Turn 1
            ...
            ├── user_prompt.txt      # The user's prompt for this turn
...
-   **`_context.log`**: The single source of truth for what the AI was shown in a given turn. It includes the full content of all resources specified in `turn.context`. This file is for auditing and reproducibility.
-   **`system_prompt.xml`**: A copy of the full system prompt used to generate the plan for this turn.
-   **`user_prompt.txt`**: A file containing the user's prompt message for this turn.
...
-   **Behavior:**
    ...
    2.  **State Check:**
        -   If the turn is complete (has a `report.md`), it **must prompt the user for a new message**, save it to `user_prompt.txt` for the new turn, and then proceed to the planning phase.
