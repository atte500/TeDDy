...
- [2026-04-16] Since the hang occurs even with `xdist` disabled (`-n 0`), it is likely caused by a leaked background thread or `asyncio`/`anyio` event loop from a previous test (suspected: Textual `App` instances leaving unresolved worker threads) deadlocking Pytest's fixture teardown/setup on Ubuntu workers.
