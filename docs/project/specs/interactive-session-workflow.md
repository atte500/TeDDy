...
The system uses a top-level `.teddy/` directory to store all persistent data. Session data is organized within a dedicated `sessions/` subdirectory to keep the root clean. To prevent context drift, the agent-specific system prompt resides in the session root.
...
-   **`{agent_name}.xml`**: The system prompt providing the core instructions for the active agent (e.g., `pathfinder.xml`).
...
-   **Behavior:**
    ...
    3.  Reads the content of the newly generated `input.md` and the existing `{agent_name}.xml` prompt from the session root.
    4.  Passes the `input.md` and `{agent_name}.xml` files to the LLM.
...
3.  **Initialize Next Turn (`T_next`):**
    ...
    -   **Default State:** `T_next` starts as a direct continuation of `T_current`.
        -   Deprecated: Prompt cloning is disabled; agents fetch from session root.
...
-   **`turn_context_threshold`**: (Optional) An integer defining the token budget for the active working set (`turn.context`) before auto-pruning is triggered. This threshold applies ONLY to the sum of files in the `Turn` scope; it excludes `session.context` (Session scope) and system prompts (System scope). Defaults to `50000`.
