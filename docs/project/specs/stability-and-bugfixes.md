...
- **403 Bypassing:** The `WebScraperAdapter` must attempt to bypass 403 Forbidden errors by rotating User-Agents or using common headers.
...
- **Deduplication:** Ensure final context items are deduplicated and that resource contents are not redundantly added to reports in session mode (they are already present in the turn's `input.md`).
