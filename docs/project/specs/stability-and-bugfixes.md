...
- **Pruning Logic:** The `turn_context_threshold` config key defines the token budget for Turn-scope files only (originating from `turn.context`). The calculation MUST only sum the token counts of files in the `Turn` scope. Files from the `Session` scope (`session.context`) and the `System` scope (system prompts) are excluded from the budget check, although they remain part of the final payload.
