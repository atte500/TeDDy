...
## Deliverables
- [ ] **Contract** - Ensure `preserve_message_turns: true` is supported in `config.yaml` and default service settings.
...
- [ ] **Logic** - Update `SessionPruningService` to strictly respect the `preserve_message_turns` configuration.
