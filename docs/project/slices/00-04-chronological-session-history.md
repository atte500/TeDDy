...
- [x] **Logic** - Update `ContextService._format_content` to partition workspace files from session files, formatting workspace files in `## 4. Resource Contents` and session files in `## 5. Session History` (sorted chronologically and filtered for recognized turns).
- [x] **Wiring** - Update `build_context_section` in `src/teddy_executor/adapters/inbound/textual_plan_reviewer_helpers.py` to use the helpers, filtering out all `.teddy/sessions/` files from "Session" and "Turn" lists and rendering recognized history turns under a new italicized `History:` node using `HISTORY_LABEL`.
- [x] **Wiring** - Update `_is_context_data` in `src/teddy_executor/adapters/inbound/textual_plan_reviewer_logic.py` to accept `HISTORY_LABEL`.
- [x] **Wiring** - Update `populate_context_detail` in `src/teddy_executor/adapters/inbound/textual_plan_reviewer_helpers.py` to add a separate `• History` row, summing up selected history items, and only count non-session files in `• Session` and `• Turn` rows.
- [x] **Verification** - Add unit tests in `tests/suites/unit/core/services/test_context_service.py` to verify chronological formatting, exclusion from `## 4`, inclusion in `## 5`, and backward compatibility.
- [x] **Verification** - Add unit tests in `tests/suites/unit/adapters/inbound/test_reviewer_app_context_previews.py` (or similar TUI unit tests) to verify TUI `History:` rendering, styling, and aggregate updates on toggling.
...
- **Context Separation Logic**: Updated `ContextService._format_content` to partition files. Standard workspace files are formatted under `## 4. Resource Contents`, while recognized session history files are sorted chronologically and formatted under a new `## 5. Session History` section. Unrecognized session files (e.g., `meta.yaml`) are completely omitted. If no session history is found, the `## 5. Session History` section is cleanly omitted. Verified this with robust new unit tests.
- **TUI Context Tree Session Filtering & History Grouping**: Updated `build_context_section` and `format_context_item_label` in `src/teddy_executor/adapters/inbound/textual_plan_reviewer_helpers.py` to filter out session files (paths in `.teddy/sessions/`) from the generic Session and Turn folders, preventing duplication and folder clutter. Created a dedicated, italicized `History:` tree folder (mapped to `HISTORY_LABEL`) to render recognized session history items, sorted chronologically with clean human-readable names. Added a corresponding unit test in `tests/suites/unit/adapters/inbound/test_reviewer_app_context_previews.py` asserting the correct tree hierarchy and label rendering.
- **TUI Context Detail Aggregates**: Updated `populate_context_detail` in `src/teddy_executor/adapters/inbound/textual_plan_reviewer_helpers.py` to dynamically isolate session history token aggregates. Normal workspace file tokens under Session and Turn are summed separately while excluding history paths. A new `• History` aggregate row displays the total token size of selected history items. Added unit test `test_reviewer_app_shows_history_row_in_context_aggregate_detail` to verify this exact token distribution and dynamic update behavior in the TUI aggregate preview pane.
