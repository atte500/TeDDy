...
- [x] **Wiring** - Ensure standard integration test suite executes and asserts both normal transitions and validation-triggered transitions behave correctly.
...
- **Integration & Wiring**: Added `test_session_orchestrator_validation_replan_preserves_user_request_integration` to `tests/suites/integration/core/services/test_session_orchestrator_validation.py`. This test mocks a complete multi-turn session run where the initial plan fails validation, triggering an automated replan turn. The test asserts that `is_replan` is properly set to `True` on the replan turn, and that the original parent `user_request` is correctly carried forward and preserved throughout the automated replan loop without being overwritten by automated system feedback messages.
