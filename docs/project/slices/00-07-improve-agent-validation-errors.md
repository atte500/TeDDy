...
- [x] **Logic** - Fix `session_service.py` `create_session()` to include available agents in its `ValueError` message
- [x] **Logic** - Update `get-prompt` command in `__main__.py` and `prompts.py` to list available prompts on error
...
- **Logic (create_session enhancement):** Updated `create_session()` in `session_service.py` to call `self._prompt_manager.get_available_agents()` when the prompt file does not exist at `.teddy/prompts/{agent}.xml`. If the available agents list is non-empty, the `ValueError` message includes `"Available agents: {', '.join(available)}"`. If the list is empty, the original fallback message `"Please run 'teddy init' to restore prompts."` is preserved. Three unit tests cover: single missing agent with available agents listed, missing agent with empty list (no agent list in message), and valid agent (no error raised). Test file: `test_session_service.py`. Note: `import pytest` was missing from the test file and was added to resolve `NameError` on first run.

- **Logic (get-prompt enrichment):** Added `list_prompt_names()` helper in `prompts.py` that scans `.teddy/prompts/` directories upward from CWD (mirroring `find_prompt_content()` traversal), returns sorted `.xml` file stems, or empty list if no directory found. Updated `get_prompt` command in `__main__.py` to call `list_prompt_names()` when prompt not found; if list is non-empty, includes `"Available prompts: {', '.join(available)}"` in error message; if empty, uses generic not-found message. Four unit tests cover: normal listing, missing directory, empty directory, and non-XML filtering. Updated acceptance test `test_get_prompt_fails_for_non_existent_prompt` to set up `.teddy/prompts/` with two prompts and assert enriched message. Test files: `test_prompts.py`, `test_cli_ux_improvements.py`.
