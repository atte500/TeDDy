...
- **Telemetry Styling:** Wrapped the planning progress message in `SessionOrchestrator._display_planning_progress` with `[cyan]` tags, and the telemetry strings in `SessionPlanner._display_planning_telemetry` with `[dim]` tags.
- **ConsoleInteractor Adapter Refactoring:** Replaced the legacy `typer.echo` call in `ConsoleInteractorAdapter.display_message` with `self._console.print()` from `rich.console.Console` to enable native parsing and rendering of Rich style tags generated by the core services. Tests relying on `typer.testing.CliRunner` correctly capture the output of `rich.Console(stderr=True).print()`.
