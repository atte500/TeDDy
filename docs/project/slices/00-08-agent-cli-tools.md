...
When I execute `teddy run fetch-gh-log.py [ID] "[STEP]"`
...
When I execute `teddy run vcp.py -m "feat: my changes" --test "pytest"`
...
- [ ] **Logic (CLI)** - Implement dynamic script resolution: `teddy run` must locate the script in `.teddy/scripts/` (falling back to execution if it's a global binary) and execute it, passing along any trailing arguments.
...
- [ ] **Feature** - Create bundled script `src/teddy_executor/resources/scripts/vcp.py` supporting `-m`, `-t`, and `--no-verify`.
- [ ] **Migration (Log Fetcher)** - Edit `src/teddy_executor/resources/prompts/debugger.xml` to replace the `awk` one-liner with `teddy run fetch-gh-log.py [ID] "[STEP]"`.
- [ ] **Migration (VCP)** - Edit **ALL** agent prompts in `src/teddy_executor/resources/prompts/*.xml` to replace the raw bash Version Control Protocol block with the new `teddy run vcp.py` command.
...
- **VCP Token Protection:** Truncate test failure output to the last 50-100 lines to strictly protect the LLM context window from stack-trace blowouts.
- **VCP Pre-commit Logic:** If `pre-commit` returns a non-zero exit code, check `git status --porcelain`. If files were modified (meaning the linter auto-fixed them), auto-run `git add .` and continue. Only halt the script on hard errors (non-zero exit and no modified files).
- **VCP Process Safety:** When invoking Git, pass arguments as a list (e.g., `["git", "commit", "-m", message]`) to `subprocess.run` to absolutely guarantee that shell-injection and quote-escaping bugs are eliminated.
