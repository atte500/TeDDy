...
**Goal:** Implement the forward-looking side-effects for context-modifying actions and the rich reporting needed for the session workflow.
...
-   [ ] **Task: Refactor for Rich Report Generation:**
    -   Refactor the `MarkdownReportFormatter` to generate a "Rich Report" for the session workflow.
    -   This report must include the full context payload (`input.md` content), including the repo tree, file contents, and system information.
    -   For validation failures, the rich report must also include the full content of the invalid `plan.md`.
