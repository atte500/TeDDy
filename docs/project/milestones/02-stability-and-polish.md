...
- **Resume Model Override for Pending Plans:** When `teddy resume --model <override>` is called on a session in `PENDING_PLAN` state (interrupted plan), the model override is written to meta.yaml but has no effect on the existing plan's execution. The plan was generated with the old model and is executed as-is by `SessionLifecycleManager.resume()`. Fix: re-generate the pending plan using the new model before execution when a model override is provided. (See [Case File 24](/docs/project/debugging/24-model-override-on-resume.md) for analysis.)
- **DI Resolution Pattern Audit (Proposed):** Review `OrchestratorPorts` / `ExecutionOrchestrator` resolution pattern. The `ActionExecutor` is registered as `transient` but is resolved once per session inside `OrchestratorPorts`, creating a de facto singleton that caused the hash-persistence bug. Evaluate whether `OrchestratorPorts` should be freshly resolved per turn, or if stateful services should be explicitly reset.
