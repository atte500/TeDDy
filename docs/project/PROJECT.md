...
- **2026-08-29:** Silent error swallowing in `_restore_foreground_process_group()` and `_restore_terminal_cooked_mode()` in `textual_plan_reviewer_editor.py`. Both functions use `except Exception: pass` to suppress errors during terminal state restoration. While this prevents crashes, it violates the Failure Transparency rule and masks failures that could cause the TUI editor bugs documented in Case File #33. Scheduled for resolution with the Debugger fix for Bug #33.
