...
*   [ ] [Slice 09: Enhance `edit` Action Safety](./slices/09-enhance-edit-action-safety.md)
...
- **CLI Documentation Drift (from Slice 09):** The `docs/adapters/inbound/cli.md` document is outdated. It describes a `stdin`-only interface from the walking skeleton and does not reflect the current command structure that has since been implemented. This documentation needs to be updated to prevent confusion for future development.
- **Inconsistent Test Helpers (from Slice 09):** During the implementation of the `edit` action safety feature, it was discovered that the `run_teddy` helper function in acceptance tests is inconsistent. Some newer tests incorrectly tried to invoke the CLI with a file path, while older, working tests correctly use `stdin`. A future refactoring slice should standardize this helper, likely by moving it to a shared `conftest.py`, to improve test maintainability and prevent test-harness-related bugs.
