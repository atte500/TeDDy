...
4.  **Rich Feedback:** When `FIND` blocks do not match, the validator uses a **Multi-Layered Heuristic** (Exact Anchors -> Incremental Fuzzy Cascade -> Sub-sampling) to identify the closest match while maintaining sub-second performance on large files. This diagnostic diff allows the AI to self-correct even when precise line numbers or context are missing.
