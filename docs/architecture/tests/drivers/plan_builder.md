...
Acting as the **Driver** in the Test Harness, the `MarkdownPlanBuilder` is a fluent interface designed to simplify the creation of complex, multi-action Markdown plans in tests. It abstracts the boilerplate of Markdown formatting (headers, rationale, code block fences) and ensures that generated plans adhere to the TeDDy protocol.
...
- `add_edit(path: str, find_replace: Union[str, List[Tuple[str, str]]], replace: str = None, description: str = "Editing file", match_all: bool = False) -> self`: Implements `EDIT`. Supports single or multiple FIND/REPLACE pairs.
