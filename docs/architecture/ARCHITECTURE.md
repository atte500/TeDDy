...
| **SessionOrchestrator**      | A wrapper service implementing the "Turn Transition Algorithm" around the base execution logic.                        | [SessionOrchestrator](./core/services/session_orchestrator.md)                 |
...
-   **Rule:** The `EXECUTE` action supports "Internal Redirection" for `cd` and `export` commands. **Rationale:** To allow AI agents to use "natural" shell setup syntax (e.g., `cd dir && ls`) in Markdown while ensuring cross-platform compatibility by internally translating these setup commands into the appropriate `subprocess` parameters (`cwd`, `env`).
