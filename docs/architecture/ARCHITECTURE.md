...
-   **Rule:** The `EXECUTE` action prohibits shell chaining and inline directives. **Rationale:** To ensure atomicity, observability, and cross-platform consistency. All environment preparation (e.g., `cd`, `export`) MUST be placed in the optional `Setup` metadata parameter of the `EXECUTE` block rather than the command block itself.
