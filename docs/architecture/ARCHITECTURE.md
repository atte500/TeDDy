...
- **Run Environment:** All execution commands MUST be prefixed with the project's designated runner ('uv run').
...
- **Post-commit Execution:** The full test suite runs in a post-commit hook as an unskippable safety net. The hook is defined in `.pre-commit-config.yaml` with `stages: [post-commit]`. The hook script lives at `.githooks/post-commit.py` (cross-platform Python). If tests fail, the commit is automatically reverted via `git reset --soft HEAD~1`, keeping changes staged for investigation. This ensures `--no-verify` cannot bypass the test gate.
