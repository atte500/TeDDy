...
-   **Rule:** Any diagnostic logic involving sequence matching (like fuzzy `EDIT` matching) MUST use tiered heuristics (Exact Anchors -> Incremental Fuzzy -> Sub-sampling) combined with **Priority Capping**. **Rationale:** "Priority Capping" limits the number of expensive evaluations (e.g., `difflib.SequenceMatcher.ratio()`) to the top-scoring candidates (typically the top 5) identified by faster heuristics. This prevents unpredictable worst-case performance on ambiguous or highly repetitive inputs and maintains sub-second response times.
