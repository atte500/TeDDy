...
-   **Cross-Platform Shell Execution (Sequential Multi-Command Runner):** To ensure consistent "halt-on-error" behavior across platforms, the `ShellAdapter` decomposes scripts into a sequence of atomic commands.
    -   **Behavior:** Multi-line scripts are split into individual lines, and lines containing `&&` operators are further decomposed. Each resulting atomic command is executed sequentially.
    -   **Directives:** Lines or segments starting with `cd ` or `export ` (and `set ` on Windows) are intercepted and used to update the persistent execution context (`cwd` and `env`) in Python.
    -   **Halt on Error:** If any command in the sequence returns a non-zero exit code, execution stops immediately. The `ShellAdapter` returns the accumulated output and the failure code of the first failing command.
    -   **Platform Strategy:**
        -   **POSIX:** Uses `shell=True` for each atomic command.
        -   **Windows:** Uses the "Smart Router" for each atomic command.
    -   **Constraint:** This sequential approach does not support multi-line shell constructs (e.g., `if/then` blocks, loops). Complex logic must be moved to a dedicated script file.
    -   **Rationale:** This strategy mimics Unix `set -e` behavior cross-platform, ensuring that every step in a chain is subject to the adapter's failure logic and user-supervised execution report.
