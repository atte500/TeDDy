...
- **Post-commit Execution:** The full test suite runs in a post-commit hook as an unskippable safety net. The hook is defined in `.pre-commit-config.yaml` with `stages: [post-commit]` and installed via `pre-commit install --hook-type post-commit`. The hook script lives at `.githooks/post-commit`. If tests fail, the commit is automatically reverted via `git reset --soft HEAD~1`, keeping changes staged for investigation. This ensures `--no-verify` cannot bypass the test gate.
