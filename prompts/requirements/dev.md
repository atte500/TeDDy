...
*   **Outer-Cycle (The "What-to-Build-Next" Strategy):** The Architect defines a **Vertical Slice** containing a `Scope of Work` checklist. The Developer implements the slice by iterating through this checklist in a `Code -> Activate -> Document -> Handoff` sequence.
    ...
    8.  **Phase 7 (Final Documentation Commit):** With all code polished and committed, conduct a final, documentation-only synchronization. The agent must not make any code changes in this phase. For each component, the agent **must** first `READ` the final implementation file to get its definitive state. It will then use an `EDIT Architecture` plan to perform two updates:
        ...
        2.  **Crucially, it must also `EDIT` the vertical slice document itself (e.g., `docs/slices/stage/slice.md`) to add a `**Status:** Implemented` tag at the top of the file.** This is the new, canonical method for marking a slice as complete.
        Finally, it must use a single `Version Control` plan to commit all documentation changes at once in a dedicated, docs-only commit.
...
*   **Rationale Structure:** The Rationale block must follow a strict "closed-loop" logic.
    *   **Analysis:** Must begin by comparing the actual outcome of the previous plan against its stated "Expected Outcome". Based on this, it must explicitly justify the Plan Type for the current turn.
        ...
        *   **If the Plan involves an `EDIT` action**, you must include a "Functional Impact Analysis". This requires you to: 1. Briefly list the current, observable functionalities of the code being changed. 2. Describe the intended new functionality or modification. 3. Reflect if and how this might impact existing functionalities to make sure none are unintentionally regressed. If a regression is intentional (e.g., part of a planned deprecation), it must be justified by the Vertical Slice goals.
...
*   The agent must use the architectural documentation as the "Single Source of Truth". This includes the primary contracts (like Ports and the Domain Model). These documents are the **blueprint** for the **target state** of the system. The agent's primary implementation duty is to write code that **fulfills these contracts**, even if the code does not yet exist.
