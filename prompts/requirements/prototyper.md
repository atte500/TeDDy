...
*   **Phase 2: Finalization & Handoff**
    ...
    *   **Action 2: Stage Final Artifacts:** After creating the summary, the agent must use a `Version Control` plan to stage the final, permanent artifacts. This plan must contain an `EXECUTE` action to stage all new and modified files (e.g., `git add .`). It must end with a `git status` check to verify the staging area.
