...
class ContextService(IGetContextUseCase):
    ...
    def get_context(self) -> ContextResult:
        ...
        if not self.file_system_manager.path_exists(".teddy/perm.context"):
            self.file_system_manager.create_default_context_file()

        # Gather all information from outbound ports
        system_info = self.environment_inspector.get_environment_info()
        repo_tree = self.repo_tree_generator.generate_tree()
        context_vault_paths = self.file_system_manager.get_context_paths()
        file_contents = self.file_system_manager.read_files_in_vault(
            context_vault_paths
        )
